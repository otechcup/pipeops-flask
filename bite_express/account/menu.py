...
from .forms import vendor_menu_model, menu_form_model
from bite_express.settings.current_biteexer import current_biteexer
from bite_express.dbmodel import BiteVendorMenu
from bite_express.permission.checker import (
    has_setup_vendor_account, is_biteexer, is_vendor, is_restricted_on_pt
)


@menu.route("", methods=["POST", "GET"])
class Menus(Resource):
    @menu.marshal_list_with(vendor_menu_model)
    @jwt_required
    def get(self):
        """
        Get a list of all vendor menu and menu of a specific Vendor
        """
        ...
        is_restricted = is_restricted_on_pt(biteexer, ["Vendor"])
        
        if is_restricted:
            vendor_menus = (
                BiteVendorMenu.query
                    .filter_by(kitchen_name=biteexer.vendor.kitchen_name)
                    .all()
            )
        else:
            vendor_menus = BiteVendorMenu.query.all()
    ...
    def post(self):
        ...
        if _is_vendor[0]:
            ...
            has_not_setup_vendor_account = has_setup_vendor_account(biteexer)
            ...
            return make_response(jsonify(_is_vendor[1]), 401)


@menu.route("/detail/<string:bitem_id>", methods=["PUT", "GET", "DELETE"])
class Menu(Resource):
    @menu.marshal_with(vendor_menu_model)
    @jwt_required
    def get(self, bitem_id):
        ...
        """
        try:
            vendor = BiteVendorMenu.query.filter_by(bitem_id=bitem_id).one()
